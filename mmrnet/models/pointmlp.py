...
class LocalGrouper(nn.Module):
    def __init__(self, channel, groups, kneighbors, use_xyz=True, normalize="center", **kwargs):
        ...
        if self.normalize not in ["center", "anchor"]:
            log.info(f"Unrecognized normalize parameter (self.normalize), set to None. Should be one of [center, anchor].")
            self.normalize = None
...
class PointMLP(nn.Module):
    def __init__(self, info, embed_dim=64, groups=1, res_expansion=1,
        ...
        stacks = 1 if info['stacks'] is None else info['stacks']
        self.points = info['max_points'] * stacks

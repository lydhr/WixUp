...
class MMFiDataset(BaseDataset):
    # CONFIG_NAME = "mars"
    max_points = 30 # Point cloud population.
    num_keypoints = 17 # the complexity of the human skeleton in the keypoint dataset.
    ...
    def _process_keypoint(self, data_list):
        data_list = [{'x': d[0], 'y': d[1]} for d in data_list] # [x, y, action, subject, scene]
        return data_list
    
    def _process_identification(self, data_list):
        data_list = [{'x': d[0], 'y': d[3]} for d in data_list] # subject int 1- 40
        return data_list

    def _process_action(self, data_list):
        data_list = [{'x': d[0], 'y': d[2]} for d in data_list] # action int 1-27
        return data_list
